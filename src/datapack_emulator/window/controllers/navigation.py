...
class NavigationController(Controller):
    ...
    def source_menu(self, point: QPoint) -> None:
        ...
        rename = menu.addAction("rename function… (F2)", window.editor.rename_function)
        rename.setEnabled(self.function_at(self.source_path) is not None)
        menu.addAction("complete here (Ctrl+Space)", edit.ask_completions)
        menu.addSeparator()
        function_id = self.function_at(self.source_path)
